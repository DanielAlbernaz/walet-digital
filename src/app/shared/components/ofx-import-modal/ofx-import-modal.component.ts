...
@Component({
  selector: 'app-ofx-import-modal',
  templateUrl: './ofx-import-modal.component.html',
  styleUrls: ['./ofx-import-modal.component.scss']
})
export class OfxImportModalComponent implements OnChanges {
  @Input() isOpen = false;
  ...
  onFileSelected(event: Event): void {
    ...
    reader.onload = () => {
      try {
        ...
        if (parsed.length === 0) {
          this.parseError = 'Nenhuma despesa (débito) encontrada no arquivo OFX.';
          this.rows = [];
        } else {
          this.rows = this.mapParsedToPreviewRows(parsed);
        }
        this.isLoading = false;
      } catch {
        this.parseError = 'Não foi possível ler o arquivo OFX. Verifique o formato.';
        this.rows = [];
        this.isLoading = false;
      }
      input.value = '';
  ...
  private mapParsedToPreviewRows(
    parsed: ReturnType<typeof parseOfxContent>
  ): OfxImportPreviewRow[] {
  ...
  /** Marca grupos completos (1..N no arquivo) como elegíveis para parcelamento */
  ...
  goToPreview(): void {
    ...
    if (this.rows.length === 0) {
      this.toastr.warning('Importe um arquivo OFX com despesas.', 'Atenção');
      return;
    }
    this.applyTargetMonthToRows();
  ...
  isGroupPlanEnabled(groupKey: string): boolean {
    const group = this.getGroupRows(groupKey);
    return group.length > 0 && group.every((r) => r.createAsInstallmentPlan);
  }
  ...
  private buildObservation(row: OfxImportPreviewRow, isPlan: boolean): string {
    const parts = [`Importado via OFX`, `FITID: ${row.fitid}`];
    if (row.originalMemo !== row.description) {
      parts.push(`Memo original: ${row.originalMemo}`);
    }
    if (row.isInstallment && row.portion) {
      parts.push(`Parcela: ${row.portion}`);
    }
    if (isPlan) {
      parts.push('Parcelamento criado a partir do grupo no arquivo OFX');
    }
    return parts.join(' | ');
  }
